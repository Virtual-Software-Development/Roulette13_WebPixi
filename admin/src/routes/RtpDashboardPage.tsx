...
import { fetchRtpSetting } from '../api/rtpSettings'
import { RTP_GAMES } from '../data/rtpGameLabels'
import type { RtpBand, RtpMetricCardData } from '../types/rtpDashboard'
...
//
// metrics/bands arrancan en los mocks (RTP_METRIC_CARDS/RTP_BANDS) para que la pantalla nunca se
// vea vacía, y se actualizan en segundo plano con el target/min/max REAL de GET
// /rtp-settings/{juego} para los juegos que ya tienen configuración (hoy, solo roulette) -- current/
// status/House Margin/Manual Overrides/trend/Recent Changes siguen siendo mock: no existe ningún
// endpoint para esos todavía.
export function RtpDashboardPage() {
  const { t } = useTranslation()
  const now = useNow()
  const [metrics, setMetrics] = useState<RtpMetricCardData[]>(RTP_METRIC_CARDS)
  const [bands, setBands] = useState<RtpBand[]>(RTP_BANDS)
  ...
  useEffect(() => {
    ...
    Promise.all(RTP_GAMES.map((game) => fetchRtpSetting(game))).then((results) => {
      ...
      setMetrics((prev) =>
        prev.map((card) => {
          const i = RTP_GAMES.indexOf(card.id)
          const api = results[i]
          return api ? { ...card, target: api.targetRtp } : card
        }),
      )

      setBands((prev) =>
        prev.map((band) => {
          const i = RTP_GAMES.indexOf(band.id)
          const api = results[i]
          return api ? { ...band, min: api.minBand, max: api.maxBand, widthPercent: Math.round((api.maxBand - api.minBand) * 10) / 10 } : band
        }),
      )
    })
    return () => {
